...
if platform.system() == "Windows":
    ...
    # path = os.path.abspath("libtr.so")
    try:
        _libc = ctypes.cdll.LoadLibrary('./libtr.so')
    except:
        path = os.path.abspath('../libtr.so')
        _libc = ctypes.cdll.LoadLibrary(path)
...
#virget 修改推理字符沾粘问题,同一个字符至少相隔2个-1
def _parse1(unicode_arr, prob_arr, num):
    txt = ""
    prob = 0.
    count = 0
    for pos in range(num):
        ...
        if unicode >= 0:
            ...
            if pos == 0:
                add = True
            elif pos == 1 and unicode != unicode_arr[0]:
                add = True
            elif pos > 1 and unicode != unicode_arr[pos-1] and unicode != unicode_arr[pos-2]:
                add = True
